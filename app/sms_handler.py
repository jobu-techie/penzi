...
def parse_age_range(age_range_text: str):
    ...
    if len(parts) != 2:
        raise ValueError("Age range must be in format min-max")

    try:
        min_age = int(parts[0].strip())
        max_age = int(parts[1].strip())
    except ValueError as exc:
        raise ValueError("Age range values must be numbers") from exc
...
def handle_start_command(message: str):
    ...
    gender_input = gender_text.strip().capitalize()
    ...
    user.phone_number = f"TEMP_{user.id}"
    db.session.commit()
...
def handle_match_command(user_id: int, message: str):
    ...
    try:
        age_range_min, age_range_max = parse_age_range(age_range_text.strip())
    except ValueError as exc:
...
def handle_describe_command(message: str):
    ...
    return {
        "message": f"{user.name} describes themselves as {description.description}",
        "user": {
            "id": user.id,
            "name": user.name,
...
def handle_phone_interest_command(user_id: int, message: str):
    ...
    return {
        ...
        "notify_target": {
            "message": f"Hi {target.name}, {requester.name} is interested in you. "
